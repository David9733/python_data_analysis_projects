...
- **유튜브 댓글로 드러나는 감정 반응 패턴**: '인사이드 아웃' 영상 댓글 68개를 수집한 결과, 성장하면서 슬픔이에 공감하게 됐다는 반응, 빙봉을 향한 향수, 2편 관람 전 복습 목적 등 시청자의 감정적 반응이 다양하게 분포함을 확인. 댓글 분석을 통해 콘텐츠가 시청자에게 미치는 감정적 영향력을 파악할 수 있음
- **네이버 연관검색어로 파악하는 검색 의도**: '파이썬' 검색 시 연관검색어 대부분이 독학·기초·자격증·학원 등 학습 목적 키워드로, 실무 개발자보다 입문자 수요가 검색 트렌드를 주도함을 확인. '윌슨 파이썬'처럼 동명이인(연예인) 키워드가 혼재해 데이터 수집 시 노이즈 필터링이 필요함을 시사
