...
공공 데이터 분석, 웹 크롤링, DB 연동, 머신러닝 클러스터링을 포함한 9개의 Python 데이터 분석 프로젝트 모음
...
정형 데이터(CSV, Excel) 분석부터 Selenium 기반 웹 자동화, SQL을 통한 DB 조회, 머신러닝 클러스터링까지 데이터 분석의 전 과정을 직접 구현하는 것을 목표로 했다. 각 프로젝트는 실제 공개 데이터와 웹 데이터를 활용해 데이터 수집부터 결과 도출까지 독립적으로 수행했다.
...
| # | 프로젝트 | 데이터 출처 | 형식 |
|:---:|---|---|:---:|
| 1 | 가장 많이 사용된 이름 TOP 10 분석 | 미국 아기 이름 공공 데이터 | CSV |
| 2 | 평균연봉 상위 10개 사업장 확인 | 국민연금공단 공공데이터포털 | CSV |
| 3 | 지하철 데이터 KMeans 클러스터링 | 서울 열린데이터광장 (2024년 4월) | Excel |
...
- **정형 데이터**: CSV/Excel 파일 3종
- **크롤링 데이터**: 댓글 68건, 채널 랭킹, 베스트셀러 24건, 연관검색어 10건
- **DB 데이터**: MariaDB 테이블 2종 (상가 통계, 고객 정보)
...
| 사업장명 | 사업장 이름 |
| 가입자수 | 국민연금 가입 직원 수 |
| 당월고지금액 | 해당 월 국민연금 고지 금액 |

**서울 지하철 시간대별 승하차 데이터**

| 컬럼 | 설명 |
|---|---|
| 승하차일합계 | 하루 전체 승하차 인원 합계 |
| 승차_출근시간 / 하차_출근시간 | 출근 시간대 승하차 인원 |
| 승차_퇴근및저녁 / 하차_퇴근및저녁 | 퇴근·저녁 시간대 승하차 인원 |
| 승차_새벽시간 외 6개 시간대 | 새벽·점심·오후·밤 시간대 각 승하차 인원 |
...
- **인코딩 처리**: 한국 공공데이터 CSV의 `cp949` 인코딩 명시적 지정
- **파생 컬럼 생성**: 국민연금 데이터에서 인당 고지금액 계산 → 요율 역산으로 월 급여 추정 → 연봉 환산
...
| # | 프로젝트 | 결과 |
|:---:|---|---|
| 1 | 미국 아기 이름 TOP 10 | Michael(729,890회)이 1위, James · Robert · John · David 순 |
| 2 | 평균연봉 상위 사업장 | 국민연금 요율 역산 기준 상위 10개 사업장 추출 |
| 3 | 서울 중구 카페 상가수 | 총 888개 |
| 4 | 지하철 클러스터링 | 상위 20개 역을 이용 패턴 기준 3개 그룹(9개·6개·5개 역)으로 분류 |
| 5 | YES24 베스트셀러 | 실시간 베스트셀러 24건의 도서명·저자·출판사·출판일 수집 |
| 6 | 유튜브 댓글 수집 | '인사이드 아웃' 영상 68건 댓글 수집 및 DataFrame 저장 |
...
- **공개 데이터로 연봉 추정**: 국민연금 고지금액과 법정 요율(9%)을 활용하면 연봉을 직접 제공하지 않는 공개 데이터에서도 사업장별 급여 수준을 근사할 수 있음
- **서울 중구 카페 포화도**: 서울 중구에만 888개의 커피점·카페가 운영 중으로, 상업 밀집 지역의 높은 카페 포화도를 수치로 확인
