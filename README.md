...
데이터 분석, 웹 크롤링, DB 연동, 머신러닝 클러스터링을 포함한 9개의 Python 데이터 분석 프로젝트 모음
...
정형 데이터(CSV, Excel) 분석부터 Selenium 기반 웹 자동화, SQL을 통한 DB 조회, 머신러닝 클러스터링까지 데이터 분석의 전 과정을 구현하는 것을 목표로 했다. <br>각 프로젝트는 실제 공개 데이터와 웹 데이터를 활용해 데이터 수집부터 결과 도출까지 독립적으로 수행했다.
