...
| 6 | YES24 베스트셀러 | 실시간 베스트셀러 24개 도서의 도서명·저자·출판사·출판일 수집 |
| 7 | 유튜브 채널 데이터 수집 | 상위 3개 채널(MrBeast·Cocomelon·김프로KIMPRO) 순위·카테고리·구독자·조회수·영상수 수집 |
| 8 | 유튜브 댓글 수집 | '인사이드 아웃' 영상 댓글 68개 수집 및 DataFrame 저장 |
| 9 | 네이버 연관검색어 추출 | '파이썬' 검색 시 연관검색어 10개 자동 추출 |
...
- **DB 서브쿼리 기반 동적 조회**: `WHERE join_date = (SELECT MAX(join_date) ...)`처럼 서브쿼리를 활용하면 데이터가 추가되어도 항상 최신 가입 고객을 정확히 반환하는 쿼리 패턴 구현 가능
...
- **동적 페이지 댓글 수집 한계**: YouTube는 스크롤 이벤트로 댓글이 동적 로드되므로, END키 반복 입력으로 렌더링을 유도한 후 스냅샷을 수집하는 방식으로 68개 댓글 추출 성공
- **네이버 검색 자동화 확장성**: Selenium으로 검색어 입력부터 연관검색어 파싱까지 자동화하여, 검색어만 바꾸면 임의 키워드의 연관검색어를 즉시 추출 가능한 재사용 구조 확보
